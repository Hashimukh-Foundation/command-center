...
export default function Dashboard() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-zinc-950 overflow-y-auto h-full text-zinc-100">
      
      {/* RESPONSIVE WRAPPER: Limits width on large screens and centers the content
      */}
      <div className="w-full max-w-6xl mx-auto flex flex-col pb-6 md:pb-12">
        
        {/* Sleek Header Section */}
        <div className="p-6 md:px-8 md:py-10 bg-zinc-950 border-b border-zinc-800 shrink-0">
          <div className="flex justify-between items-start">
              <div className="flex items-center gap-4 md:gap-6">
                  {/* Modern Sharp Avatar - Scales up on desktop */}
                  <div className="w-14 h-14 md:w-20 md:h-20 bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xl md:text-3xl font-bold text-zinc-400">
                      {firstName.charAt(0)}
                  </div>

                  <div>
                      <h1 className="text-xl md:text-3xl font-semibold leading-tight text-white">
                          {firstName} {lastName}
                      </h1>
                      <p className="text-sm md:text-base text-zinc-500 capitalize mt-0.5 md:mt-1.5">
                        {displayRole}
                      </p>
                  </div>
              </div>
              ...
              <button 
                ...
                className="p-2.5 md:p-3 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors"
        ...
        {isEditing && (
          <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-sm z-50 flex flex-col justify-center p-4 overflow-y-auto">
            <div className="bg-zinc-950 border border-zinc-800 w-full max-w-lg mx-auto shadow-2xl relative my-auto">
              ...
              <form onSubmit={saveProfile} className="p-5 flex flex-col gap-5">
                <div className="pb-5 border-b border-zinc-800">
                  ...
                  <input type="password" placeholder="Leave blank to keep current" className="w-full bg-zinc-900 border border-zinc-800 text-white px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder-zinc-600" value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                </div>
        ...
        {/* Sleek Stats Grid - Expands spacing on desktop */}
        <div className="grid grid-cols-2 gap-4 p-4 md:px-8 md:py-6 md:gap-6">
          <div className="bg-zinc-900/50 border border-zinc-800 p-5 md:p-8 flex flex-col justify-between">
              <div className="text-zinc-500 mb-4 md:mb-6">
                  <Wallet size={24} className="md:w-8 md:h-8" />
              </div>
              <div>
                <p className="text-xs md:text-sm font-medium text-zinc-500 uppercase tracking-wider mb-1 md:mb-2">My Contribution</p>
                <p className="text-2xl md:text-4xl font-semibold text-white">৳{myDonations}</p>
              </div>
          </div>
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 p-5 md:p-8 flex flex-col justify-between">
              ...
              <div>
                <p className="text-xs md:text-sm font-medium text-zinc-500 uppercase tracking-wider mb-1 md:mb-2">Global Fund</p>
                <p className="text-2xl md:text-4xl font-semibold text-white">৳{totalCollected}</p>
              </div>
          </div>
        </div>

        {/* Main Ledger Section */}
        <div className="px-4 md:px-8 flex-1 mt-2 md:mt-4">
          
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 gap-4">
            <h3 className="text-sm md:text-base font-medium text-zinc-400">Activity for {formattedMonth}</h3>
            
            {/* Action Button - Moves next to title on desktop */}
            {isAdminOrTreasurer && (filter === 'unpaid' || filter === 'partial') && filteredMembers.length > 0 && (
              <button 
                onClick={handleBulkReminder}
                disabled={sendingBulk}
                className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white py-3 md:py-2 md:px-6 font-medium flex items-center justify-center gap-2 transition-colors disabled:opacity-50 text-sm md:text-base"
              >
                <Bell size={18} />
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2 md:pb-0 md:flex-wrap hide-scrollbar">
            <button onClick={() => setFilter('all')} className={`px-4 md:px-6 py-2 md:py-2.5 border text-xs md:text-sm font-medium whitespace-nowrap transition-colors ${filter === 'all' ? 'bg-white text-zinc-950 border-white' : 'bg-transparent text-zinc-400 border-zinc-800 hover:border-zinc-700 hover:text-zinc-200'}`}>
              All Members
            </button>
            <button onClick={() => setFilter('cleared')} className={`px-4 md:px-6 py-2 md:py-2.5 border text-xs md:text-sm font-medium whitespace-nowrap transition-colors ${filter === 'cleared' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/50' : 'bg-transparent text-zinc-400 border-zinc-800 hover:border-zinc-700 hover:text-zinc-200'}`}>
              Cleared
            </button>
            <button onClick={() => setFilter('partial')} className={`px-4 md:px-6 py-2 md:py-2.5 border text-xs md:text-sm font-medium whitespace-nowrap transition-colors ${filter === 'partial' ? 'bg-amber-500/10 text-amber-500 border-amber-500/50' : 'bg-transparent text-zinc-400 border-zinc-800 hover:border-zinc-700 hover:text-zinc-200'}`}>
              Partial
            </button>
            <button onClick={() => setFilter('unpaid')} className={`px-4 md:px-6 py-2 md:py-2.5 border text-xs md:text-sm font-medium whitespace-nowrap transition-colors ${filter === 'unpaid' ? 'bg-rose-500/10 text-rose-500 border-rose-500/50' : 'bg-transparent text-zinc-400 border-zinc-800 hover:border-zinc-700 hover:text-zinc-200'}`}>
              Unpaid
            </button>
          </div>

          {/* Clean List View - Turns into a beautiful Card Grid on Desktop */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 md:gap-4 border-t border-zinc-800 md:border-none divide-y divide-zinc-800 md:divide-none bg-zinc-900/20 md:bg-transparent">
            {filteredMembers.map(m => (
              <div key={m.id} className="p-4 md:p-5 flex items-center justify-between hover:bg-zinc-800/30 transition-colors md:bg-zinc-900/40 md:border md:border-zinc-800">
                <div className="pr-4">
                  <p className="font-medium text-zinc-100 md:text-lg truncate">{m.full_name || 'Unidentified Member'}</p>
                  <p className="text-xs md:text-sm text-zinc-500 capitalize mt-0.5 md:mt-1">
                ...
                <div className="flex items-center shrink-0">
                  {m.amount_paid >= m.expected_amount ? (
                    <CheckCircle2 className="text-emerald-500 md:w-7 md:h-7" strokeWidth={2} size={22} />
                  ) : m.amount_paid > 0 ? (
                    <div className="flex items-center gap-1.5 px-2 md:px-3 py-1 md:py-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-500">
                      <span className="text-xs md:text-sm font-medium">৳{m.amount_paid} / {m.expected_amount}</span>
                    </div>
                  ) : (
                    <XCircle className="text-rose-500 md:w-7 md:h-7" strokeWidth={2} size={22} />
                  )}
                </div>
              </div>
            ))}
          ...
          {filteredMembers.length === 0 && (
            <div className="p-10 text-center text-zinc-500 text-sm md:text-base border border-zinc-800 md:mt-4 bg-zinc-900/20">
